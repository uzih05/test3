...
function AppearanceSection() {
  ...
  const themes = [
    { key: 'dark' as const, label: t('settings.theme.dark'), icon: Moon, desc: 'Neon dark mode' },
    { key: 'light' as const, label: t('settings.theme.light'), icon: Sun, desc: 'Clean light mode' },
    { key: 'system' as const, label: t('settings.theme.system'), icon: Monitor, desc: 'Follow OS preference' },
  ];
...
function AISection() {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err) => {
      setMessage({ type: 'error', text: (err as Error).message || 'Failed to save API key' });
    },
  });
  ...
  const removeMutation = useMutation({
    ...
    onError: (err) => {
      setMessage({ type: 'error', text: (err as Error).message || 'Failed to remove API key' });
    },
  });
      ...
      <div className={cn(
        ...
        {user?.has_openai_key ? (
          <>
            ...
            <span className="text-sm text-neon-cyan font-medium">OpenAI API key is configured</span>
          </>
        ) : (
          <>
            <AlertTriangle size={16} className="text-neon-orange" />
            <span className="text-sm text-neon-orange font-medium">No OpenAI API key set</span>
          </>
        )}
      </div>

      <p className="text-xs text-text-muted mb-4">
        Required for AI features: Healer diagnosis, function Q&A, and semantic search with vector embeddings.
      </p>
...
function ConnectionsSection() {
  ...
  const saveKeyMutation = useMutation({
    ...
    onSuccess: () => {
      setKeyMessage({ type: 'success', text: 'API key updated' });
      queryClient.invalidateQueries({ queryKey: ['connections'] });
      setTimeout(() => { setKeyEditId(null); setKeyMessage(null); }, 2000);
    },
    onError: (err) => {
      setKeyMessage({ type: 'error', text: (err as Error).message || 'Failed to save' });
    },
  });
  ...
  return (
    <div className="bg-bg-card border border-border-default rounded-[20px] overflow-hidden card-shadow">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-sm text-text-muted">No connections configured</div>
      ) : (
              ...
              {keyEditId === conn.id && (
                <div className="mt-3 ml-5.5 pl-3 border-l-2 border-border-default">
                  <p className="text-xs text-text-muted mb-2">
                    OpenAI API Key for <span className="text-text-primary">{conn.name}</span>
                  </p>
                  <div className="flex gap-2">
                    <input
                      type="password"
...
function AccountSection() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Security info */}
      <div className="bg-bg-card border border-border-default rounded-[20px] p-6 card-shadow">
        <h2 className="text-sm font-medium text-text-secondary mb-4">Security</h2>
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Shield size={14} className="text-text-muted" />
              <span className="text-sm text-text-primary">OpenAI API Key</span>
            </div>
            <span className={cn(
